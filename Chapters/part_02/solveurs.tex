...
L'Univers est modélisé par différents "fluides" (cf section \ref{cosmoparam}) et l'objectif des simulations numériques est d'estimer au mieux leurs évolutions, en fonction des lois auxquelles ils sont soumis et de leurs états à un instant donné.
La résolution de chacun de ces systèmes sera confiée à différents "moteurs", ayant pour tâche de résoudre les systèmes d'équations différentielles gouvernant les différentes physiques.
...
Pour la simuler on utilisera le principe des code N-corps, c'est-à-dire qu'on utilisera un champ de particules massives interagissant uniquement par gravitation.
...
Elle consiste à ne pas calculer directement la force, mais à la dériver à partir du potentiel, lui même calculé sur une grille.
...
On associe généralement une taille à la particule similaire à celle des cellules sur laquelle on la projette.
...
Cette méthode est itérée jusqu'à ce que soit le potentiel ait convergé, soit un nombre maximal d'itérations soit atteint.
...
Elle consiste à séparer la grille en un damier de deux couleurs, une première itération sera réalisée seulement sur la partie rouge, puis une seconde itération sera réalisée sur la partie noire en prenant en compte le nouvel état de la partie rouge. 
...
\end{equation}
où $|v_{max}|$ représente la vitesse de la particule la plus rapide (de l'ordre de $100$ km.s$^{-1}$ dans nos régimes typiques d'échelles et de résolutions).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\clearpage
\section{Hydrodynamique et physique baryonique}
\label{sec:hydro}
%solveur hydro
...
Dans le cas de l'étude de l'\ac{EoR} la physique du gaz est centrale car l'objectif est précisément de déterminer les propriétés physico-chimiques de ce gaz et comment elles impactent la formation des galaxies.
...
En première approximation, le gaz dans l'Univers est principalement soumis à deux effets : sa pression interne et la gravité.
...
avec la densité $\rho = \rho(\vec{x},t)$,
...
\end{equation}
où les indices représentent les dérivées temporelles et spatiales.
...
où: $\Lambda = \Lambda_{(E,\rho)}$ est la fonction de refroidissement et $H$ est le photo chauffage (cf section \ref{sec:chimie}).
...
Considérant un système, régi par des équations de conservations, ayant comme état initial deux milieux homogènes de densités constantes séparés par une discontinuité, qu'elle sera son évolution?
...
Son approche consiste à estimer le flux aux interfaces des cellules et à discrétiser l'équation \ref{eq:conservation} de la manière suivante : 
...
où $C \in [0,1] $ est le nombre de Courant et $V = max(v + c_s)$ une estimation de la vitesse de l'onde la plus rapide présente dans la simulation \citep{toro1999riemann} avec $v$ la vitesse du gaz et la vitesse du son $c_s = \sqrt{\gamma P/\rho}$ dans le cas d'un gaz parfais.
...
Le pas de temps hydrodynamique est gouverné par la vitesse maximum du gaz (de l'ordre de $3000$km.s$^{-1}$) et sera en pratique près d'un ordre de grandeur plus faible que le pas de temps N-corps.
...
\item La seconde famille est dite de Monte Carlo.
...
Les codes LICORICE \citep{semelin_lyman-alpha_2007} ou CRASH \citep{2003MNRAS.345..379M} utilisent cette méthode.
...
EMMA utilise une approximation du moment au premier ordre et son moteur repose sur l'\textit{approximation M1} \citep{levermore_relating_1984, dubroca1999theoretical}.
...
et $S_\nu = \dot{N}_\nu^* + \dot{N}_\nu^{rec}$ le taux d’émission de photon dû aux sources $\dot{N}_\nu^*$ et à la recombinaison $ \dot{N}_\nu^{rec}$.
...
De la même manière que l'équation d'état des gaz parfait est utilisée pour fermer le système d'équations d'Euler (voir \ref{sec:hydro}), dans le cas de l'approximation M1, la fermeture du système se fait par l'intermédiaire de l’équation d’état :
...
Les valeurs intermédiaires permettent d'approximer la superposition de ces deux régimes.
...
J'ai également pu tester le cas de trois groupes, dans ce cas les bornes sont généralement prises au niveau des énergies d'ionisation de l'hélium : 
$\nu_2= 24.6 eV$, $\nu_3= 54.4 eV $, $\nu_4= + \inf$.
...
Vu que le coût global du transfert du rayonnement augmente rapidement avec le nombre de groupes et dans le cas des résolutions considérées pendant ma thèse, l'augmentation du nombre de groupes ne s'est pas révélée pertinente.
...
$H = c n_H \Sigma_E N$ est le terme de photo-chauffage.
...
Les processus chimiques agissant sur des échelles de temps courtes par rapport au transport des photons (en particulier lors de l'utilisation de la RSLA, cf section \ref{sec:RSLA}).
le pas de temps pourra parfois être plus court que le pas de temps radiatif.
%sera plusieurs ordres de grandeurs plus faible.
...
Si celle ci est supérieure à 10\% ($\Delta E/E>0.1$), le pas de temps est divisé par 2 et l'étape courante est recalculée. (cf \cite{rosdahl_ramsesrt_2013}) 
...
Pour assurer une bonne reproduction du régime linéaire, l'expansion de la grille sera limitée d'un pas de temps à l'autre \citep{teyssier_cosmological_2002}.
...
\begin{equation}
\Delta t_{simu} = min \left( \Delta t_{pic}, \Delta t_{hydro}, \Delta t_{rad}, \Delta t_{cosmo}\right)
\end{equation}
...
De plus comme les niveaux fins évoluent avant les grossiers, les flux aux interfaces doivent être traités proprement : les cellules grossières "stockent" les flux provenant des niveaux fins avant  d'être complètement mis à jour.
...
Dans notre cas, le rayonnement est plusieurs ordres de grandeurs plus contraignant que l'hydrodynamique ou la gravitation.
Dans ce cas le nombre de pas de temps hydrodynamiques et gravitationnels nécessaires sont largement surestimés, et le coût numérique explose.
...
La densité de photon calculée sur la grille de base est ensuite projetés sur les niveaux raffinés pour effectuer la chimie, ces derniers ressentant localement un champ uniforme de rayonnement.
...
Un amélioration possible serait de pouvoir définir le niveau de projection du rayonnement pour pouvoir, en fonction des besoins, faire varier le poids du rayonnement dans le calcul.
