...
Le défis des simulations de la réionisation est qu'elles doivent simuler un volume d'Univers suffisant grand pour être représentatives de la variance cosmique( $\approx 100$ Mpc selon \cite{iliev_cosmological_2006}) tout en résolvant la formation stellaire (échelle $<1$ pc).
...
Dans EMMA la distribution des domaines utilisés pour la parallélisation est réalisée à l'aide d'une courbe de Peano-Hilbert.
...
Ce type de découpage minimise les interfaces entre domaines, en s'assurant d'une proximité spatiale entre tout les points de la courbe.
...
Ainsi les vecteurs pourraient être en permanence sur le \ac{GPU}, et seul l'information sur leurs positions dans l'\ac{AMR} devrait être transmise.
...
Une des évolutions possible de EMMA, serait l'utilisation de courbe de Morton ou Z-order curve \citep{morton1966computer}.
...
L'avantage la courbe de Morton est que la détermination de l'indice de la cellule et très simple et bien adaptée aux \ac{GPU}.
...
De plus, ces données sont généralement calculé sur des machines distantes et améliorer leur compacité peut permettre de gagner du temps au moment du transfert vers des machines locales, ou simplement au moment de la lecture pour analyse.
