...
EMMA est une code à grille adaptétive \ac{AMR} capable de simuler l'évolution de la matière noire, du gaz et de la radiation de manière auto cohérente et dans un contexte cosmologique.
...
J'ai été amené à interagir avec toutes les parties du code, et à en modifier quelques unes.
...
Dans la troisième partie je présenterai quelques contraintes matérielles dans l'exécution d'EMMA et également quelques développements que j'ai réalisé, entre autre une amélioration de la gestion des entrées/sortie.
...
D'une manière générale, un code \ac{AMR} offre une meilleur conservation de l’énergie mais pourra souffrir de problèmes de diffusion.
...
Également, l'\ac{AMR} est plus apte à gérer les chocs et les instabilités mais sera plus diffusif.
...
Pour créer la grille de base, il faudra d'abord créer une première cellule, appelé racine, et le forcer à raffiner.
...
Comme la grille est amenée à évoluer, il n'y a plus il n'existe plus de lien direct entre une position dans l'espace et une position en mémoire.
...
Un OCT se trouve toujours entre une CELL "mère" et huit CELLs "fille".
...
La génération de conditions périodiques (généralement utilisées en cosmologie) se fait de manière naturelle en faisant pointer tous les voisins de l'OCT racine vers lui même.
La structure minimale d'un OCT de EMMA est présenté sur le listing \ref{lst:oct}.
...
Les processus physiques que l'on cherche a résoudre sont régis par des systèmes d'équations différentielles. (voir section \ref{sec:solvers})
