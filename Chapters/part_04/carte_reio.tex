...
Cette étude se base sur les cartes de redshift de réionisation, un outils qui permet de condenser toute l'histoire d'ionisation d'une simulation.
...
L'augmentation du taux d'ionisation prévaut alors, et la fraction de neutre résiduelle diminue.
...
L'instant à conserver sera définis comme l'instant de passage de la fraction d'ionisation de la cellule par un seuil.
...
Il s'avère également que c'est le taux d'ionisation considérer par \cite{planck_collaboration_planck_2016} pour déterminer le redshift de réionisation de l'Univers.
...
Les fronts peuvent atteindre des vitesses plus élevée faisant augmenter la moyenne.
...
En réduisant encore la vitesse (histogramme du bas, $\tilde{c}=0.01$), non seulement le pic est coupé, mais la première partie à vitesses constantes ($z>8$) commence à être impactée.
Dans cette phase, certains fronts ont déjà des vitesses supérieures à $\tilde{c}$, est sont limitées dès le début du processus.
...
Comme nous l'avons vu en section \ref{sec:vreio}, il existe deux cas pouvant mener à une détection de vitesse biaisée, l'une d'entre elles étant la rencontre de deux fronts d'ionisation.
...
Au final, l'outil de calcul de la vitesse des fronts d'ionisation a été appliqué ici au cas de l'étude de la \ac{RSLA} mais pourrait être utilisé pour tirer des conclusions plus astrophysique, en étant appliqué aux simulations présentées dans la prochaine section pour étudier l'influence de l’environnement cosmologique sur la propagation de l'ionisation dans le Groupe Local par exemple.
