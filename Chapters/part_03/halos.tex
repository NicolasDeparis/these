\chapter[Rétro-action stellaire]{Influence de la rétro-action stellaire sur les propriétés des galaxies dans les simulations de la Réionisation}
\label{sec:galaxies}
...
Un effet similaire est observé en faisant varier la quantité d'énergie injectée par les supernovae.
...
Cette partie fait également référence à la publication présentée en annexe \ref{pap:feedback} et a pour objectif de présenter les méthodes de détection des galaxies, pour pouvoir ensuite mener une série d'études statistiques.
...
Les galaxies étant situées au sein de halos de matière noire, on détectera les halos dans un premier temps, puis on y associera la partie baryonique représentant les galaxies.
...
J'ai utilisé un paramètre de taille de lien caractéristique de valeur standard $0.2$ et un nombre de particules minimum de 10 menant à la détection d'environ 20000 halos par simulation.
...
A l'aide d'un arbre généré sur les étoiles, j'ai associé à chaque halo toutes les étoiles à une distance inférieure à son $R_{200}$ autour de son centre de masse.
...
Cette méthode est évidemment bien plus longue qu'une simple recherche dans la sphere, et ne sera appliquée qu'aux halos les plus asphériques, ces derniers étant généralement les plus massifs, puisque c'est l'association de plusieurs sur-densités qui mène à une forte dissymétrie.
...
A ce stade, nous avons défini la position et la taille de tous les halos et défini les galaxies par association.
...
De plus on observe dans le champ de densité des coquilles dues aux vents générés, autour du halo central dans le cas du modèle cinétique.
...
On observe un léger décrochage de la \ac{HMF} pour les plus petits halos ( $M< \mathrm{qq} 10^8 M_\odot$), ce décrochage est attendu et apparaît pour les halos résolus avec moins de 100 particules.
...
Par souci de simplicité, nous utiliserons l'approximation du $R_{200}$.
Il faut donc discrétiser la sphère de rayon $R_{200}$ autour de chaque halo.
Cette discrétisation sera réalisée grâce à Healpix \citep{gorski_healpix:_2005}, un outil qui permet de répartir des points de manière régulière sur la sphère (c.f. figure \ref{fig:HealPix}).
%L'association Healpix/grille sera réalisée par recherche de plus proche voisin 
Par la suite, toutes les surfaces des halos seront discrétisées en utilisant $3072$ points Healpix.
...
%Si il est possible de travailler directement avec les grandeurs scalaires, 
Lors d'une analyse vectorielle (l'étude des flux par exemple) les vecteurs seront projetés sur la normale à la surface.
...
Cette population possède des étoiles %ayant déjà explosées en supernovae, 
ce qui suggère un effet provenant de l'intérieur.
...
De plus, le feedback cinétique permet de générer des flux sortants sur des halos plus massifs que dans le cas du feedback thermique.
Les vitesses moyennes sont proches de la vitesse d'échappement mais elles restent en quasi intégralité sous cette limite.
Dans ces modèles de feedback, le gaz est expulsé des halos mais toujours gravitationnellement lié à eux.
On mesure également une forte augmentation du bruit dans la population de halos avec des vitesse moyennes négatives : l'introduction d'énergie par les supernovae induit un brassage du gaz environnant les halos qui limite l’effondrement du gaz sur ceux-ci. 
...
Le nombre de photons utiles à l'ionisation de l'\ac{IGM} étant peu variable, cela explique pourquoi l’évolution de la fraction d'ionisée n'est pas impactée.
Il doit donc y avoir un effet opérant entre les régions de formation stellaire internes aux halos et leurs $R_{200}$ qui fait évoluer la fraction d'échappement des photons.
...
Comme il a été mentionné dans la section \ref{sec:sfr_halo}, cette gamme de masse de halos est à la limite d'être convergé du fait de la taille restreinte des simulations considérées ici.
La décroissance observée dans le budget de photons pour ces masses et relativement faible et lors de la convergence une chute d’émissivité plus brutale est normalement attendue.
%et la taille de ces simulations joue sûrement un rôle dans ces conclusion.
Il est probable que dans des simulations plus grandes, disposant de halos plus massifs, le maximum du budget de photons soit décalé vers des masses plus importantes.
...
La résolution des grandes simulations actuelles n'est pas suffisante et il me semble nécessaire d'augmenter la résolution en masse ainsi que la résolution radiative de un voir deux ordres de grandeurs, pour résoudre la physique des halos jusqu'à des masses de l'ordre de $M \approx 10^8M_\odot$ et ainsi étudier ce changement de régime de manière plus assurée, tout en gardant un volume simulé suffisamment grand de l'ordre de $\left( 100 \cdot h^{-1} \mathrm{cMpc} \right)^3$.
