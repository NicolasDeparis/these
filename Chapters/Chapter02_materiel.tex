...
La production de simulation cosmologique a haute valeur scientifique se fait sur des centres de calcul.
...
Un nœud est un élément conceptuellement proche d'un ordinateur personnel.
Il dispose d'une carte mère, avec un (ou plusieurs) CPU, de la RAM, un SSD/HDD et parfois un GPU.
...
La quantité d'information passant par une interface représente un certain coup.
On cherchera à minimiser ce coup en optimisant le transfert des données à différents niveaux.
...
Et ce d'autant plus que les neouds sont physiquement éloignés entre eux.
...
Ainsi si l'on applique ce pavage aux cellules de notre grille, en associant un indice reliant un cellule à sa position sur la courbe de Hilbert.
...
\begin{figure}[bth]
        ...
        \caption{Principe de la courbe de Hilbert. 
...
Il existe 2 principaux avantage des \ac{GPU} par rapport aux \ac{CPU}.
...
\begin{figure}[bth]
        ...
        \caption{Comparaison CPU/GPU. Un \ac{CPU} dispose d'un nombre réduit de cœurs capables d’exécuter des opération complexe. 
...
C'est à dire que leurs espaces mémoire ne sont pas commun.
Une code hybride est découpé en deux parties, une partie séquentielle exécuté sur le \ac{CPU} et une partie parallèle exécuté sur le \ac{GPU}.
...
C'est à dire que les calculs sur GPU seront plus performants si les données sont organisées de façons à ce que leurs accès mémoire consécutifs soient physiquement proches.
...
La difficulté est que l'arbre AMR ne respecte pas cette organisation, et peut être extrêmement fragmentée à cause de la liste chainée.
...
En passant à la cellule suivante, la cellule précédente sera de nouveau copié et au final le potentiel d'une cellule sera donc copié sept fois sur le GPU.
...
Il existe des techniques optimisé de gestion d'arbre sur GPU a base de "Z-order curves", mais son implémentation nécessite une refonte en profondeur du code.
...
Cette simulation fait partie des plus grosse simulation de la réionisation exécutée avec un nombre de particule de matière noire, et donc une grille coarse, de $2048^3$ éléments .
Et si il faut calculer ces données, il faut également les stocker et les écrire sur disque dur.
...
Mais on voudrait évidemment avoir accès a l'état de la simulation a différents instant et cette simulation a au final générée 170 snapshot.
...
De plus, ces données sont généralement calculé sur des machines distante et améliorer leur compacité peut permetre de gagner du temps au moment du transfert vers des machines locales, ou simplement au moment de la lecture.
...
Dans l'ancien modèle de données d'EMMA, l'intégralité de l'octree était écrit.
...
Dans la même optique que ce qui a été abordé dans la section \ref{sec:cpugpu}, j'ai implémenté un modèle se basant sur une notion de tableau de champs.
L'objectif est de sortir les données de l'arbre pour faciliter leurs traitement.
Dans ce nouveau modèle, seules les feuilles (les cellules non raffinées) sont écrites sur le disque, ce qui permet de réduire la redondance des données.
En contrepartie, il est nécessaire de reprojeter la grille dans le cas d'une analyse sur un niveau intermédiaire.

Les champs sont maintenant traité individuellement et il est possible de choisir de les écrire ou non en fonction des besoins.
...
Malgré la quantité de données réduite, le temps dédié a sortir les données n'en est que très peu affecté.
Le gain réalisé à l'écriture est compensé par la charge de calculs supplémentaires due à l'extraction des données de l'arbre.
